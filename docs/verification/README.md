...
- [SMOKE-TOURNAMENT-END.md](SMOKE-TOURNAMENT-END.md) - End-of-tournament bench state: when TODAY and UPCOMING are both empty, a designed broadcast sign-off card stands in for the empty tabs, with the real signed-player count and links to replays, the bracket, and results; RESULTS still works and the card vanishes the moment a fixture returns.
